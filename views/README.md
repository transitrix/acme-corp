...
```
views/goals/eu-strategy.goals.transitrix.yaml
views/dgca/eu-expansion.dgca.transitrix.yaml
...
Atomic elements — Goals, Capabilities, BusinessProcesses (thin metadata files), Products, ApplicationComponents, etc. — stay in `canon/elements/<layer>/`. The view in `views/products/` is a *filter expression* over many Product elements, each of which lives in `canon/elements/02_business/<id>.yaml`.
